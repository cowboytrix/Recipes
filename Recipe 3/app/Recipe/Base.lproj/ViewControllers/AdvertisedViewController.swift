...
class AdvertisedViewController: UIViewController, GADBannerViewDelegate {
    ...
    func createBannerView() -> GADBannerView {
        ...
        if UIScreen.main.bounds.size.height == 812.0 { // detect iphone X and adjust to it
            bannerView = GADBannerView(adSize: bannerAdSize, origin: CGPoint(x: 0, y: self.view.frame.height - bannerHeight - 22))
        } else {
            bannerView = GADBannerView(adSize: bannerAdSize, origin: CGPoint(x: 0, y: self.view.frame.height - bannerHeight))
        }
